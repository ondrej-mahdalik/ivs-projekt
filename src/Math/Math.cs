    ...
    public class MathClass
    {
        ...
        internal static int Prec(char ch)
        {
            switch (ch) {
                case '-':
                    return 0;
                case '+':  // Addition could have the same precedence as subtraction, but this way it corresponds to the way most people (and online tools) would perform the conversion (doesn't change the results)
                    return 1;
        ...
        public static string InfixToPostfix(string input)
        {
            ...
            if (input[0] == '-') { //Leading negative number
                int numEndIndex = 1;
                while(numEndIndex < input.Length && (char.IsLetterOrDigit(input[numEndIndex]) || input[numEndIndex] == '.')) {
                    numEndIndex++;
                }
            ...
            foreach(char op in operators) {
                while (input.Contains(op + "-")) { //Negative number after an operator
                    ...
                    while (loopIndex < input.Length && ( char.IsLetterOrDigit(input[loopIndex]) || input[loopIndex] == '.' ) ) { //Gets to the end of the negative number
                        loopIndex++;
                    }
            ...
            int currentFunc = -1; //Represents wheter c is inside of a function(Abs,Log,Root) and which one. -1 = Not in a function, 0 = in Abs, 1 = in Log, 2 = in Root (corresponds with indexes of specialFuncs)
            ...
            for (int i = 0; i < input.Length; i++) {
                char c = input[i];
                if (Char.IsLetterOrDigit(c) || c == '.') { // Character is operand (number or constant) or one of (Abs, Log, Root)
                    if (Char.IsLetter(c)) {   //( π is considered a letter)
                        if (c == 'π' || c == 'e' || c == 'E') {
                            ...
                            for(int j = 0; j < specialFuncs.Length; j++) {
                                 if (c == specialFuncs[j]) currentFunc = j;
                            }
                            if (i != 0 && !operators.Contains(input[i - 1]) && input[i - 1] != '(') { // Handle implicit multiplication ( 3Log(10) => 3*Log(10) )
                                stack.Push('*');
                                postfix += " ";
                            }
                            stack.Push('[');
                    ...
                    if (!ignoreOpeningBracket) { //Not a bracket of a function
                        if(i != 0 && !operators.Contains(input[i - 1]) && input[i - 1] != '(') { // Implicit multiplication of brackets ( 3(2) => 3*(2) )
                            stack.Push('*');
                            postfix += " ";
                        }
                        stack.Push(c);
                    ...
                    else { //Pop from stack to output until '(' is encountered
        ...
        public static double FromString(string input)
        {
            ...
            for (int i = 0; i < postfix.Length; i++) {
                ...
                if(char.IsDigit(c) || c == '.') { // C is a part of a number
                    ...
                    switch (c) { // Performs the given operation on numbers on top of the stack and pushes the result to the stack
                        case '+':
                            operand2 = stack.Pop();
                            operand1 = stack.Pop();
